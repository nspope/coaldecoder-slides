...
def fetch(i, itt):
    if WHICH_MODEL == "osclog":
        ...
        return tskit.load(f"/sietch_colab/natep/trio-coal/sims/osclog-alt/singer-snakemake/results/osclogalt_{i}/trees/osclogalt_{i}.{itt}.trees")
...
def calculate_rates(ts_list, time_grid, num_blocks=10, random_seed=1, pairs_only=True):
    ...
    for i in ts.individuals():
        sample_population[i.nodes] = population_map[i.metadata['population']]
    ...
    for ts in ts_list:
        ...
        if rates_calculator is None:
            ...
            rates_calculator.join(calculator(ts, sample_sets, time_grid, sample_set_names=population_names, bootstrap_blocks=windows))
