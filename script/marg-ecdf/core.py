...
def average_ecdf(ts):
    pop_index = { tree.tree_sequence.population(i).metadata['name'] : i for i in range(ts.num_populations) }
    sample_sets = [
...
def draw_tree(axs, tree, root_length, point_x):
    ...
    for n in tree.postorder():
        ...
        if tree.is_leaf(n):
            if labeling[n] == "CEU":
                axs.plot(point_x, node_y[n], marker="o", markersize=1, color="firebrick", label="CEU")
            elif labeling[n] == "CHB":
...
def draw_pair(axs, x, y, labels=None, label_color=None, color='black'):
    ...
    if labels is not None:
        ...
        tip_labels += [axs.text(xmin, ymax + 0.01, labels[0], va='bottom', ha='center', color=label_color[0], size=7, transform=axs.transAxes)]
        tip_labels += [axs.text(xmax, ymax + 0.01, labels[1], va='bottom', ha='center', color=label_color[1], size=7, transform=axs.transAxes)]
    return tip_labels
